"""
Fix for UUID type mismatch in work_experience table
This script fixes the error:
"operator does not exist: character varying = uuid"
by ensuring the user_id column in work_experience is properly UUID type
"""

import uuid
from sqlalchemy import text
from database import SessionLocal, engine
from models import Base, WorkExperience

def fix_work_experience_uuid_type():
    """Fix the UUID type mismatch in work_experience table"""
    print("Starting UUID type fix for work_experience table...")
...
def verify_fix():
    ...
    with SessionLocal() as db:
        try:
            ...
            result = db.execute(
                text("SELECT * FROM work_experience WHERE user_id = :user_id"),
